    ...
    http.createServer(function(request, response) {
        ...
        function handle(request, response) {
            ...
            router.route(request, function(err, moduleName, controllerName, actionName, query) {
                if ( err ) { throw new Error(err); } // TODO report to client somehow...
                console.log('routed: ' + request.url, 'to', err, moduleName, controllerName, actionName, query);
                serveRequest(err, db1, request, moduleName, controllerName, actionName, query, response, responseCallback);
            });
        }
    }).listen(8888);
}
...
/**
 * origRequest: { url, query, body }
 */
function serveRequest(err, db, origRequest, moduleName, controllerName, actionName, query, response, responseCallback) {
        var internalRequest = { 
            db: db, 
            moduleName: moduleName, controllerName: controllerName, actionName: actionName, 
            orig: origRequest, url: origRequest.url, query: query,
...
exports.start = start;
exports.serveRequest = serveRequest; // exported so we can use it for testing from the command line in js/cli/client.js
