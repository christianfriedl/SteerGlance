...
function list(bo, request, response, responseCallback) {
    ...
    var conditions = [];
    bo.loadAllByConditions(conditions, function(err, bos, aggregateBo) {
        // if ( err ) return responseCallback(err); TODO
        
        bo.countByConditions(conditions, function(err, count) {
            // if ( err ) return responseCallback(err); TODO
...
function saveField(bo, request, response, responseCallback) {
    ...
    if ( typeof(request.body.row.id) !== 'undefined' && request.body.row.id.length > 0 ) {
        bo.loadById(request.body.row.id, function(err) {
            ...
            bo.saveField(request.body.fieldName, request.body.row, function(err, bo2, flags) { // NOTE: alternate callback sig for additional return info!
                if ( err ) throw err;
                afterSaveCallback(bo, response, flags);
            });
        }, false);
    } else {
        bo.saveField(request.body.fieldName, request.body.row, function(err, bo2, flags) { // NOTE: alternate callback sig for additional return info!
    ...
    function afterSaveCallback(bo, response, flags) {
...
function count(bo, request, response, responseCallback) {
    bo.countByConditions([], function(err, count) {
        if ( err ) throw err;
