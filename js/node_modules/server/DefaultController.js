...
function saveField(entitySet, request, response) {
    if ( request.body.id !== undefined ) {
        ...
        }).then((state, messages, errors) => {
            return _.merge(response, { state: state, messages: messages, errors: errors });
        }).then((response) => {
            return entity.toJson());
        }).then((row) => {
            return _.merge(response, { row: row, id: row.id });
        });
        }); // TODO error handling
    } else {
        throw 'insert';
        /* TODO insert
        entity.saveField(request.body.fieldName, request.body.row, function(err, bo2, flags) { // NOTE: alternate callback sig for additional return info!
            if ( err ) throw err;
            return entity.toJson().then((row) => {
