...
const m_sql_conditionSet = require('sql/ConditionSet.js');
...
function edit(bo, request, response, responseCallback) {
    bo.loadById(request.query.id, function(err) {
        ...
        var data = { 
            action: 'edit', 
            form: 'edit',
            row: {fields: bo.fieldsToJson(), id: bo.id() }
        };
        ...
        function(bos, aggregateBo, callback) {
            boSet.countByConditions(conditions, function(err, count) {
                ...
                response = _(response).extend({
                    ...
                    form: 'list',
                    data: {
                        rows: _(bos).map(function(bo) { return {fields: bo.fieldsToJson(), id: bo.id() }; }),
                        aggregateRow: { fields: aggregateBo.fieldsToJson() }, // all fields, so we can determine where each one goes
                        count: count,
                        templateRow: {fields: boSet.templateBo().fieldsToJson() }
                    }
                });
...
function saveField(bo, request, response, responseCallback) {
    console.log('dc savefield request body', request.body);
    if ( typeof(request.body.id) !== 'undefined' && request.body.id.length > 0 ) {
        bo.loadById(request.body.id, function(err) {
            if ( err ) throw err;
            bo.saveField(request.body.fieldName, request.body.row, function(err, bo2, flags) { // NOTE: alternate callback sig for additional return info!
                if ( err ) throw err;
                response.data = { row: { fields: bo.fieldsToJson(), id: bo.id() }, flags: flags };
                responseCallback(response);
            });
        }, false);
    } else {
