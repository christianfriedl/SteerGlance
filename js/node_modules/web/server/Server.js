...
Server.prototype._handleRequest = function(httpRequest, httpResponse) {
    ...
    const internalResponse = { 
        returnCode: 200, 
        controller: routeDesc.controllerName,
        action: routeDesc.actionName,
    ...
    try {
        ...
        method(internalRequest, internalResponse).done((internalResponse) => {
            ...
            const text = this._getResponseText(internalResponse);
            console.log('final response text (first 200 chars)', text.substring(0, 200));
...
Server.prototype._getResponseText = function(response) {
    switch ( response.contentType ) {
        case 'text/json':
            return JSON.stringify(deps._.merge({ controller: response.controller, action: response.action,  }, response.json));
        case 'text/html':
        default:
