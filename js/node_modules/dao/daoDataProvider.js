...
DaoDataProvider.prototype.field = function(fieldOrName) {
    if ( fieldOrName instanceof m_sql_field.Field ) { // TODO we really need to genericize this!!!!!!!!
        ...
        if ( typeof(fieldOrName.onAfterAddToDao) === 'function' ) { // so the field can replace its link fields with the new fields
            fieldOrName.onAfterAddToDao(this);
        }
        return this;
    }
    return this._fields[fieldOrName]; // here it is the name
};
...
DaoDataProvider.prototype._fieldsFromTable = function(table) {
    ...
    _.each(_.keys(tf), function(key) { 
        this.field(tf[key].clone());  // upon putting the fields into the dao, we actually clone them...
    }, this);
};
...
DaoDataProvider.prototype.writableFieldsAsList = function() {
    return _(this._fields).values().filter(function(f) { return f.className() !== 'sql.CalcField'
        && f.className() !== 'sql.BoField' && !(f instanceof m_sql_boField.BoField) }); // TODO this condition is not exactly right
};

DaoDataProvider.prototype.databaseFieldsAsList = function() {
    return _(this._fields).values().filter(function(f) { 
        return !(f instanceof m_sql_calcField.CalcField || f instanceof m_sql_boField.BoField); 
    }); // TODO this condition is not exactly right
};
