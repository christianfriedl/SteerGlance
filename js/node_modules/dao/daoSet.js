...
DaoSet.prototype.willCalculateDetailCalcFields = function(willCalculateDetailCalcFields) {
...
DaoSet.prototype.willCalculateAggregatedCalcFields = function(willCalculateAggregatedCalcFields) {
 ...
 */
DaoSet.prototype.loadAllByConditions = function(/* m_sql_conditionSet.ConditionSet */ conditions, /* Function */ callback) {
    /* @DEVELOPMENT { */
    ...
    var calculateDetailCalcFields = this._willCalculateDetailCalcFields;
    var self = this;
    /*
    ...
    var aggregateDao = this.templateDao();
    if ( this._willCalculateAggregatedCalcFields ) {
        aggregateDao.calculateCalcFields(conditions, this.loadDetailRows.bind(this));
    } else {
        this.loadDetailRows.bind(this)(false, undefined);
    }
};

/** 
 * determine whether an object instantiated by this DaoSet would be primary or not
 */
DaoSet.prototype._isDaoPrimary = function() {
    return ( this._constructor.call({}) instanceof m_dao_primaryDao.PrimaryDao ); 
};

DaoSet.prototype.loadDetailRows = function(err, aggregateDao) {
    if ( err ) { throw new Error('loadDetailRows failed'); }

    calculateDetailCalcFields = this._willCalculateAggregatedCalcFields && this._isDaoPrimary();
        ...
        if ( calculateDetailCalcFields ) {
            async.each(daos, 
                (dao, callback) => {
                    ...
                    async.parallel([
                        function(callback) { return dao.calculateCalcFields(singleBoConditions.filters(), callback); },
                        function(callback) {  // TODO phewww, this and the above might be dependent on each other under certain circumstances...?
                            return callback(err, dao);
                        }
                    ], (err) => { 
                        return callback(err, daos, aggregateDao); 
                    });
                }, function(err) {
                    return callback(false, daos, aggregateDao);
                });
        } else {
            async.each(daos, 
                (dao, callback) => {
                    return callback(false, dao);
                }, function(err) {
                    return callback(false, daos, aggregateDao);
                }
            );
...
/*
 * calculate each calcField by fetching its query, and applying it to the db to 
 * fetch the values
 */
