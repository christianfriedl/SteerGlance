...
function PrimaryDao(db) {
    if ( arguments.length > 0 ) {
        ...
        /* @DEVELOMPENT } */
        m_dao_dao.DAO.call(this, db);
        this._willCalculateCalcFields = false;
        this._willPopulateLookupFields = false;
    } else {
...
PrimaryDao.prototype.table = function(table) {
    /* @DEVELOPMENT { */ var PARAM = table; if ( !(PARAM instanceof m_sql_table.Table || typeof(PARAM) === 'undefined') ) { throw new Error('typecheck failed on ' + PARAM + ': ' + PARAM); } /* @DEVELOMPENT } */
    if ( typeof(table) !== 'undefined' ) {
        this._amendTable(table);
        return m_dao_dao.DAO.prototype.table.call(this, table);
    }
    return this._table;
};

PrimaryDao.prototype._amendTable = function(/* m_sql_table.Table */ table) {
    /* @DEVELOPMENT { */ var PARAM = table; if ( !(PARAM instanceof m_sql_table.Table) ) { throw new Error('typecheck failed on ' + PARAM + ': ' + PARAM); }  /* @DEVELOMPENT } */
    if ( typeof(table.field('id')) === 'undefined' ) {
...
PrimaryDao.prototype.willCalculateCalcFields = function(willCalculateCalcFields) {
    if ( typeof(willCalculateCalcFields) !== 'undefined' ) {
        this._willCalculateCalcFields = willCalculateCalcFields;
        return this;
    }
    return this._willCalculateCalcFields;
};

PrimaryDao.prototype.willPopulateLookupFields = function(willPopulateLookupFields) {
    if ( typeof(willPopulateLookupFields) !== 'undefined' ) {
        this._willPopulateLookupFields = willPopulateLookupFields;
        return this;
    }
    return this._willPopulateLookupFields;
};
...
PrimaryDao.prototype.loadById = function(id, callback) {
    ...
    this._db.fetchRow(q, function(err, row) { 
        ...
        condPopLookupFields(function(err, dao) {
            if ( this._willCalculateCalcFields ) {
                this.calculateCalcFields([m_sql_filter.filter(this.field('id'), m_sql_filter.Op.eq, id)], function(err, dao) {
    ...
    function condPopLookupFields(callback) {
        if ( self._willPopulateLookupFields ) {
            return self.populateLookupFields(callback);
        } else {
            return callback(false, this);
        }
    }
...
PrimaryDao.prototype._createIdSelect = function() {
    return query.select()
        .fields(this.databaseFieldsAsList()) // TODO not writable but...
        .from(this._table)
        .where(this._createIdFilter());
};

PrimaryDao.prototype._createIdFilter = function() {
    return m_sql_filter.filter(this._fields['id'], m_sql_filter.Op.eq, this._fields['id'].value());
};
