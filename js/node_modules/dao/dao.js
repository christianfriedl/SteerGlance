...
DAO.prototype.writableFieldsAsList = function() {
    var x = _(this._fields).values().filter(function(f) { return !(f.className() === 'sql.CalcField'); }); // TODO this condition is not exactly right
    console.log('writableFieldsAsList called, returning ', x);
...
DAO.prototype._fieldsFromTable = function(table) {
    var tf = table.fields();
    _.each(_.keys(tf), function(key) { this.field(tf[key].clone()); }, this);
};
...
DAO.prototype.fieldValuesFromBo = function(bo) {
    _(this.writableFieldsAsList()).each( function(f) { 
        console.log('writable in from bo field', f);
...
DAO.prototype.loadAllByConditions = function(conditions, callback, calculateCalcFields) {
    ...
    console.log('calculateCalcFields in loadAllByConditions', calculateCalcFields);
    return this.loadAllByQuery(q, function(err, daos) {
        if ( calculateCalcFields ) {
            async.each(daos, function(callback) {
                }, function(err, res) {
                    console.log('labq daos after calcfields', daos);
                    callback(false, daos);
            });
        } else {
            callback(false, daos);
...
DAO.prototype.calculateCalcFields = function(conditions, callback) {
    async.map(_(this._fields).filter(function(f) { console.log('instance in calculateCalcFields', f.name(), f instanceof m_sql_calcField.CalcField); return f.className() === 'sql.CalcField'; }), 
            function(f, callback) {
                f.conditions(conditions);
                var q = f.query();
                this._db.fetchRow(q, function(err, row) {
                    if ( err ) return callback(err);
                    console.log('dao calcfields fetchrow', row);
                    callback(false, row);
                });
            },
            function(err, row) {
                console.log('final callback called', row);
            }
