...
DAO.prototype.databaseFieldsAsList = function() {
    return _(this._fields).values().filter(function(f) { return !(f.className() === 'sql.CalcField'); }); // TODO this condition is not exactly right
};
...
DAO.prototype.loadAllByConditions = function(conditions, callback, calculateDetailCalcFields, calculateAggregatedCalcFields) {
    function loadDetailRows(err, aggregateDao) {
        if ( err ) { throw new Error('loadDetailRows failed'); }

        calculateDetailCalcFields &= ( typeof(self.loadById) === 'function' );
    ...
    var aggregateDao = this._constructor.call({}, this._db, this._table); // construct a new dao of the correct kind
    var self = this;
