...
DAO.prototype.loadAllByQuery = function(query, callback) {
    ...
    this._db.fetchAll(query, function(err, rows) {
        ...
        _(rows).each(function(row) {
            ...
            console.log('dao_loadAllByQuery setValue ', dao, ' row', row, 'id1', dao.id1());
            daos.push(dao);
        }, this);
        console.log('loadAllByQuery', daos, daos[0].id1(), daos[1].id1());
        callback(false, daos);
    }.bind(this));
};
...
function dao(db, table) { console.log('constructing new dao'); return new DAO(db, table); }
