...
var DAO = function(/*Db*/p_db, /*Table*/p_table) {

    m_dao_daoDataProvider.DaoDataProvider.call(this, p_db, p_table);
    if ( arguments.length > 0 ) {
        /* @DEVELOPMENT { */ 
            if ( arguments.length !== 2 ) { console.log('arguments.length', arguments.length); throw new Error('Illegal Arguments Length'); }
            if ( !(p_db instanceof m_sql_db.DB) ) { throw new Error('typecheck failed on p_db'); }  
            if ( !(p_table instanceof m_sql_table.Table) ) { throw new Error('typecheck failed on p_table'); }  
        /* @DEVELOMPENT } */
        ...
        async.series([
            function(callback) {
                this.populateLookupFields(callback);
            }.bind(this)]
        );
    }
};
...
function dao(db, table) { if ( arguments.length > 0 ) { return new DAO(db, table); } else { return new DAO(); } }
