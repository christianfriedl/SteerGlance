...
/**
 * EntityModel is a model for an entity 
 *
 *
 * - It is assumed that an entity has a field 'id'
 *
 ...
 * @method create
 * @param {sql_DB.DB} db
 * @param {sql_Table.Table} table
 * @return model_EntityModel.EntityModel
 */
...
/**
 * set the table
 * NOTE: we need to clone the table first
 *
 * @method setTable
 * @param {sql_Table.Table} table
 */
...
/**
 * @method setValuesFromRow
 * @param {Array} row
 */
EntityModel.prototype.setValuesFromRow = function(/* Array */ row) {
    _.each(this._table.getPersistentFields(), ( field ) => {
...
/**
 * this is async, because it will use field.getValue()!
 * @method validate
 * @async
 * @return {Promise}
 */
EntityModel.prototype.validate = function() {
    return q.all(_.map(this._table.getPersistentFields(), (field) => { return field.getValue().then((value) => { return field.validate(value, field); })}));
};
...
EntityModel.prototype.save = function() {
    return this.validate().then( () => {
        return this._table.getField('id').getValue().then( ( id ) => {
            if ( id !== null ) {
                return this._update();
            } else {
                return this._insert();
...
EntityModel.prototype._insert = function() {
    return this._db.allSql('SELECT MAX(id)+1 AS id FROM ' + this._table.getName()).then( (rows) => { // TODO this should be done via a query obj
        ...
        const query = sql_Query.insert()
            .into(this._table)
            .setFields(this.getPersistentFields())
        return this._db.run(query);
    }).catch( ( err ) => {
        console.error('error in _insert', err);
    });
};
...
EntityModel.prototype._update = function() {
    const cond = sql_Filter.create()
        ...
        .setCompareTo(this._table.getField('id').getValue());
...
/**
 * @method getPersistentFields
 * @return {Boolean}
 */
EntityModel.prototype.getPersistentFields = function() {
    return _.filter(this.getFieldsAsList(), ( field ) => { 
        return field.getIsPersistent();
    });
};
 ...
 * @method getField
 */
EntityModel.prototype.getAttribute = EntityModel.prototype.getField = function(name) {
 ...
 */
EntityModel.prototype.getAttributeNames = EntityModel.prototype.getFieldNames = function() {
 ...
 */
EntityModel.prototype.getAttributeValue = EntityModel.prototype.getFieldValue = function(name) {
