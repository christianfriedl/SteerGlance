...
EntitySetModel.prototype.getDb = function() {
    return this._db;
};
...
EntitySetModel.prototype.getTable = function() {
    return this._table;
};
...
EntitySetModel.prototype.findEntityById = function(/* int */ id) {
    ...
    const query = sql_Query.select(this._table.getFieldsAsList())
        .from(this._table)
        .setFields(this._table.getPersistentFields())
        .where(cond);
...
EntitySetModel.prototype.findEntities = function(/* sql_ConditionSet */ conditionSet) {
    const dfd = q.defer();
    console.log('findEntities conditionSet is', conditionSet);
    const query = sql_Query.select()
        .from(this._table)
        .setFields(this._table.getPersistentFields())
        .setFilters(conditionSet.getFilters())
        .setOrderBy(conditionSet.getOrderBy())
        .limit(conditionSet.getLimit())
        .offset(conditionSet.getOffset())
        // TODO set limit and offset...
    /*
        */

    console.log('findEntities query is', query);
    this._db.fetchAll(query).then( (rows) => {
        const ems = _.map(rows, (row) => {
            const em = this._entityModelConstructor(this._db, this._table);
            em.setValuesFromRow(row);
            return em;
        });
        dfd.resolve(ems);
    });
...
/**
 * create a new, empty, entity model
 */
/* model_EntityModel */ EntitySetModel.prototype.createEntityModel = function() {
    return this._entityModelConstructor(this._db, this._table);
};
