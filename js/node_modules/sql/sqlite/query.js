...
Query.prototype._insertQueryString = function() {
    ...
    sql += _(table.fields()).invoke('name').join(', ');
    sql += ') VALUES (';
    sql += _(table.fields()).map(function(field) { this._params.push(field.value()); return '?'; }.bind(this)).join(', ');
    sql += ')';
    return sql;
};
...
Query.prototype._whereClause = function() {
    ...
    sql += _.map(
        this._serverQuery.conditions(), function(c) { 
            ...
            var what = null;
            console.log('testing is_field for', fieldOrValue);
