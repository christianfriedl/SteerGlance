...
DB.prototype.allSql = function(sqlString, params) {
    ...
    this._db.all(sqlString, params, function(err, rows) {
        console.log('sqlite_db_allSql: err', err, 'rows', rows);
        if ( err ) { dfd.reject(new Error(err)); }
...
// runQuery
DB.prototype.run = function(sqlQuery) {
    var qu = query.create(sqlQuery);
    var qs = qu.getQueryString();

    return qu.getParams().then( ( p ) => {
 ...
 */
DB.prototype.fetchRow = function(sqlQuery) {
    var qu = query.create(sqlQuery);
    var qs = qu.getQueryString();
    var p = qu.getParams();
    const dfd = q.defer();
