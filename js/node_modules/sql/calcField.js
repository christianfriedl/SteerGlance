...
function CalcField(/*String*/name, /*m_sql_field.DataType*/ dataType, /* string */ label, /* CalcType */ calcType, /* OPTIONAL m_sql_field.Field */ sumField, /* OPTIONAL m_sql_field.Field */ byField) {
    if ( arguments.length > 0 ) {
        ...
        /* @DEVELOPMENT { */
            ...
            if ( !( util.isInEnum(calcType, CalcType) ) ) { throw new Error('typecheck failed on calcType: ' + calcType + ', not in ' + CalcType); }
...
function calcField(/*String*/name, /*m_sql_field.DataType*/ dataType, /* string */ label, /* m_sql_calcField.CalcType */ calcType, /* OPTIONAL m_sql_field.Field */ sumField, /* OPTIONAL m_sql_field.Field */ byField) {
    if ( arguments.length > 0 ) {
        /* @DEVELOPMENT { */
            ...
            if ( !( util.isInEnum(calcType, CalcType) ) ) { throw new Error('typecheck failed on calcType: ' + calcType + ', not in ' + m_sql_calcField.CalcT); }
            if ( ! ( typeof(sumField) === 'undefined' || sumField instanceof m_sql_field.Field) ) { throw new Error('Typecheck failure: sumField should be of class m_sql_field.Field: ' + sumField); }
            if ( ! ( typeof(byField) === 'undefined' || byField instanceof m_sql_field.Field) ) { throw new Error('Typecheck failure: byField should be of class m_sql_field.Field: ' + byField); }
        /* @DEVELOMPENT } */
        return new CalcField(name, dataType, label, calcType, sumField, byField);
    } else {
...
CalcField.prototype.clone = function() {
    var f = new CalcField(this._name, this._dataType, this._label, this._calcType);
    f.table(this._table);
    f._isEditable = false;
    ...
    switch ( this._calcType ) {
        case CalcType.sum:
            if ( typeof(this._sumField) === 'undefined' ) {
                ...
                throw new Error('calcField of calcType "sum" requires sumField to be set, this is: ' + util.inspect(this));
            }
            return this._sumQuery();
            break;
        default: 
...
CalcField.prototype._sumQuery = function() {
    var agg = m_sql_aggregate.aggregate(m_sql_aggregate.Type.sum, this._sumField);
    agg.alias(this._name);
    var link = this._byField.link(this._table);
    var q = m_sql_query.select()
        .tables( [this._table, this._sumField.table()] )
        .fields([agg])
        .where(m_sql_filter.filter(link.sourceField(), m_sql_filter.Op.eq, link.targetField()));
...
CalcField.prototype.sumField = function(sumField) {
    if ( typeof(sumField) !== 'undefined' ) {
        this._sumField = sumField;
        return this;
    }
 ...
 * getter/setter
 */
CalcField.prototype.byField = function(byField) {
