...
var sql_SumField = require('sql/SumField.js');
var sql_CountField = require('sql/CountField.js');
var sql_MinField = require('sql/MinField.js');
var sql_MaxField = require('sql/MaxField.js');
...
function create(/* String */ name) { 
    /* @DEVELOPMENT { */ 
        assert.strictEqual(arguments.length, 1, 'Illegal argument length '  + arguments.length + ', should be 1');
    /* @DEVELOMPENT } */ 
...
exports.isDatabaseField = isDatabaseField;
...
Table.prototype.getDatabaseFields = function() {
    return _.filter(this.getFieldsAsList(), ( field ) => { 
        return isDatabaseField(field);
    });
};

// TODO this should be refactored... into sql_Field, or similar; isWritable or sth
/* static boolean */ function isDatabaseField(/* sql_Field */ field) {
    return !(field instanceof sql_LookupField.LookupField) && !(field instanceof sql_ZoomField.ZoomField)
        && !(field instanceof sql_SumField.SumField) && !(field instanceof sql_CountField.CountField)
        && !(field instanceof sql_MaxField.MaxField) && !(field instanceof sql_MinField.MinField);
}

Table.prototype.clone = function() {
    const table = create(this._name);
