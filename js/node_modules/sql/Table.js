...
function create(name) { return new Table(name); }
...
Table.prototype.getField = function(name) {
    console.log('getfield for', name, 'will return', this._fields[name]);
    return this._fields[name];
};

Table.prototype.addField = function(field) {
    /* @DEVELOPMENT */ if ( !(field instanceof sql_ValueField.ValueField) ) { throw new Errors.TypeError('field must be a sql_ValueField.ValueField'); }
    field.setTable(this);
    field.setSeq(_(this._fields).values().length);
    this._fields[field.getName()] = field;
    console.log('addField for', field.getName(), 'will enter', field);
    return this;
};
...
Table.prototype.setFields = function(/* Array(sql_ValueField.ValueField) */ fields) {
...
Table.prototype.clone = function() {
    ...
    table.setFields(_.map(this.getFieldsAsList, ( field ) => { return field.clone(); }));
    return table;
};

/**
 * creates a new Table from his one, but with ValueFields instead of just Fields
 * -- useful for the entity model
 */
Table.prototype.cloneWithValueFields = function() {
    const table = this.clone();
    table.setFields(_.map(this.getFieldsAsList(), ( field ) => { 
        if ( field instanceof sql_LookupField.LookupField ) {
            return field;
        } else {
            return sql_ValueField.fromField(field); 
        }
    }));
    return table;
};
