...
function conditionsFromWeb(table, requestConditions) {
    if ( typeof(requestConditions) === 'undefined' || requestConditions === null ) {
        return [];
    }
    return _(requestConditions).map(function(c) {
        return condition(table.field(c.fieldName), opFromName(c.opName), c.value);
    });
}
