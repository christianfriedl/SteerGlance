...
BO.prototype.populateLookupFields = function(callback) {
    async.each(this.lookupFieldsAsList(), function(field, callback) {
        return this.populateLookupField(field, callback);
    }.bind(this), function(err) {
        callback(err);
    }
};
BO.prototype.populateLookupField = function(/* m_sql_lookupField.LookupField */ field, /* function */ callback) {
    /* @DEVELOPMENT { */
        if ( arguments.length !== 2 ) { throw new Error('Illegal argument length '  + arguments.length + ', should be ' + 2); }
        if ( !(fieldname instanceof string) ) { throw new Error('Typecheck failure: fieldname should be of class Class: ' + fieldname); }
        if ( !(callback instanceof Function) ) { throw new Error('Typecheck failure: callback should be of class Function: ' + callback); }
        if ( !(this.field(fieldName) instanceof m_sql_lookupField.LookupField) ) { throw new Error('Typecheck failure: this.field(fieldName) should be of class m_sql_field.Field: ' + this.field(fieldName); }
    /* @DEVELOMPENT } */

    return this._dao.populateLookupField(field, callback);
};
