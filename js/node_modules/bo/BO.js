...
BO.prototype.fieldsWebized = function() {
    return _(this.fieldsAsList()).map(function(f) { f.webize(); });
};
...
BO.prototype.loadAllByConditions = function(conditions, callback) {
    this._dao.loadAllByConditions(conditions, function(err, daos) {
        if (err) return callback(err);
        var bos = _(daos).map(function(dao) { console.log('###id1', dao.id1()); var bo = this._constructor.call({}, this._db, dao); bo._fieldValuesFromDao(dao);console.log('###id1 bo', bo.id1()); return bo; }.bind(this));
