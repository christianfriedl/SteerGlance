...
BO.prototype._fieldValuesFromDao = function(dao, withCalcFields) {
    ...
    if ( withCalcFields ) {
        fields = dao.databaseFieldsAsList(); // TODO do we really want to use dao's field list?
    } else {
        fields = dao.writableFieldsAsList();
    }
    _(fields).each(function(f) { 
        console.log('_fieldValuesFromDao doing', f.name());
