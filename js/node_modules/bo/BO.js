...
var BO = function(/*dao_dao*/p_dao) {
    m_bjoo.BJOObject.apply(this);
    if ( arguments.length > 0 ) {
        /* #ifdef DEVELOPMENT */ if ( arguments.length !== 1 ) { throw new Error('Illegal argument length'); }
...
BO.prototype.field = function(fieldOrName) {
    if ( typeof(fieldOrName._className) !== 'undefined' && (fieldOrName.className() === 'sql.LookupField' || fieldOrName.className() === 'sql.Field' || fieldOrName.className() === 'sql.CalcField') ) {
...
BO.prototype._fieldValuesFromDao = function(dao, withCalcFields) {
    ...
    _(fields).each(function(f) { this.fieldValue(f.name(), f.value()); }, this);
};
...
function bo(p_dao) { return new BO(p_dao); }
