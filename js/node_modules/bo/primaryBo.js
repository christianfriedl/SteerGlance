...
function PrimaryBo(/* m_sql_db.DB */ db) {
...
PrimaryBo.prototype.willCalculateCalcFields = function(willCalculateCalcFields) {
    if ( typeof(willCalculateCalcFields) !== 'undefined' ) {
        this._willCalculateCalcFields = willCalculateCalcFields;
        this._dao.willCalculateCalcFields(willCalculateCalcFields);
        return this;
    }
    return this._willCalculateCalcFields;
};

PrimaryBo.prototype.loadById = function(id, callback) {
    var self = this;
    /* @DEVELOPMENT { */ 
...
PrimaryBo.prototype.delete = function(callback) {
    /* @DEVELOPMENT { */ 
        if ( !(this._dao instanceof m_dao_primaryDao.PrimaryDao) ) { throw new Error('typecheck failed on this._dao: ' + util.inspect(this._dao)); }  
    /* @DEVELOPMENT } */ 
    this._dao.fieldValuesFromBo(this);
    return this._dao.delete(function(err) {
        if (err) callback(err);
...
PrimaryBo.prototype.saveField = function(/*string */ fieldName, /* object{name: value,...} */ row, callback, /*[{ autoSave: [bool true] }]*/ options) {
    ...
    console.log('primarybo savefield row', row, 'fn', fieldName);
    // return if any required field is not set in row
    if ( _(this.fieldsAsList()).any(function(f) { 
        return ( ( f.isRequired() ) && ( typeof(row.fields[f.name()]) === 'undefined' || row.fields[f.name()].length === 0 ) ); 
    }.bind(this)) ) {
        return callback(false, this, false); // NOTE: alternate callback sig for additional return info!
    }
    _(_(row.fields).keys()).each(function(name) { if ( this.field(name).isEditable() ) { this.fieldValue(name, row.fields[name]); } }.bind(this));
    this.field(fieldName).validate(row.fields[fieldName], this);
    this.fieldValue(fieldName, row.fields[fieldName]); // does auto-validate!, so if it fails, we won't move on from here
