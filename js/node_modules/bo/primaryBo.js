...
PrimaryBo.prototype.saveField = function(fieldName, row, callback, /*[{ autoSave: [bool true] }]*/options) {
    ...
    if ( _(_(row).keys()).any(function(k) { console.log('saveField require check', k, this.field(k).isRequired(), row[k]); return ( ( this.field(k).isRequired() ) && ( typeof(row[k]) === 'undefined' || row[k].length === 0 ) ); }.bind(this)) ) {
        console.log('saveField AM IN IF');
    ...
    if ( opts.autoSave ) {
        ...
        return this.save(function(err, bo) { return callback(err, this, true); });
    } else {
        console.log('saveField', 'will NOT autoSave');
        return callback(false, this, false);
    }
};
