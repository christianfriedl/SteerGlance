...
function PrimaryBo(/*dao_dao*/p_dao) {
    if ( arguments.length > 0 ) {
        /* @DEVELOPMENT { */ 
            if ( arguments.length !== 1 || p_dao === undefined ) { throw new Error('Illegal argument length'); }
            if ( !(p_dao instanceof m_dao_dao.DAO) ) { throw new Error('typecheck failed on p_dao: ' + p_dao); }  
        /* @DEVELOMPENT } */ 
        m_bo_bo.BO.call(this, p_dao);
    }
...
function primaryBo(p_dao) { if ( arguments.length > 0 ) { return new PrimaryBo(p_dao); } else { return new PrimaryBo(); } }
